...
async function copyConfigSchema() {
  ...
  // Modify and copy for .axcoderc.json
  ...
  schema.$defs.SerializedContinueConfig.properties.mergeBehavior = {
    ...
    markdownDescription:
      "If set to 'merge', .axcoderc.json will be applied on top of config.json (arrays and objects are merged). If set to 'overwrite', then every top-level property of .axcoderc.json will overwrite that property from config.json.",
    "x-intellij-html-description":
      "<p>If set to <code>merge</code>, <code>.axcoderc.json</code> will be applied on top of <code>config.json</code> (arrays and objects are merged). If set to <code>overwrite</code>, then every top-level property of <code>.axcoderc.json</code> will overwrite that property from <code>config.json</code>.</p>",
  };
