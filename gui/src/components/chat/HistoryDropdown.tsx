...
function groupSessionsByDate(sessions: SessionMetadata[]): GroupedSessions {
  ...
  Object.keys(groups).forEach((key) => {
    if (groups[key].length === 0) {
      ...
      groups[key].sort(
        (a, b) => parseInt(b.dateCreated) - parseInt(a.dateCreated),
      );
    }
  });
                      ...
                      <SessionContent>
                        {editingSessionId === session.sessionId ? (
                          <EditInput
                            ...
                            onKeyDown={(e) =>
                              handleEditKeyDown(e, session.sessionId)
                            }
                            autoFocus
                        ...
                        <SessionTime>
                          {formatTimeAgoFromTimestamp(
                            parseTimestamp(session.dateCreated),
                          )}
                        </SessionTime>
                      </SessionContent>
                      ...
                      <ButtonGroup>
                        {editingSessionId === session.sessionId ? (
                          <SaveButton
                            ...
                            variant="save"
                            onClick={(e) =>
                              handleSaveEdit(session.sessionId, e)
                            }
                            title="저장"
                          >
                            <CheckIcon />
                          ...
                          <>
                            ...
                            <DeleteButton
                              ...
                              variant="delete"
                              onClick={(e) =>
                                handleDeleteClick(session.sessionId, e)
                              }
                              title="채팅 삭제"
                            >
                              <TrashIcon />
