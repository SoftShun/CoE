...
const ChatTab = styled.div<{ isActive: boolean }>`
  ...
  background-color: ${(props) =>
    props.isActive ? "rgba(212, 168, 83, 0.15)" : "rgba(255, 255, 255, 0.08)"};
  color: ${(props) =>
    props.isActive
      ? "#D4A853"
  ...
  border: 1px solid
    ${(props) =>
      props.isActive ? "rgba(212, 168, 83, 0.3)" : "rgba(255, 255, 255, 0.1)"};
  border-radius: 5px;
  cursor: pointer;
  user-select: none;
...
export function ChatTabsSelector({
  ...
  const saveTitle = (tabId: string) => {
    ...
    if (
      trimmedTitle &&
      trimmedTitle !== tabs.find((tab) => tab.id === tabId)?.title &&
      onTabTitleEdit
    ) {
            ...
            >
              <TabContent>
                {editingTab === tab.id ? (
                  ...
                  <TabTitle
                    onDoubleClick={(e) =>
                      handleTitleDoubleClick(tab.id, tab.title, e)
                    }
